...
#   - - - http://www.365admin.com.au/2017/06/hybrid-management-part-08-creating.html        #
#                                                                                           #
#############################################################################################

$CSVLocation = "C:\Scripts\RemoteUserMailboxes.csv"

### Step 1 - User Creation

Import-CSV $CSVLocation | ForEach-Object {
New-EXLRemoteMailbox -Name $_.Name -FirstName $_.FirstName -Initials $_.Initials -Lastname $_.LastName -UserPrincipalName $_.UPN -OnPremisesOrganizationalUnit $_.OU -Password (ConvertTo-SecureString $_.password -AsPlainText -Force) -ResetPasswordOnNextLogon $false
...
Write-Host "Please wait while I add delegates with Send On Behalf permissions"
Start-Sleep -s 120
...
Write-Host "Please wait while the accounts are assigned licenses and delegate permissions applied"

Start-sleep -s 300

### Step 4 - Assign licenses to new Remote User Mailboxes and add Full Access permissions
...
Write-Host "Remote User Mailboxes created and synched to Office 365. Licenses have also been assigned"